...
class DeleteResourceForm extends AbstractType implements FileInterface
{
    /**
    ...
    {
        $form = $this->fileSystem->deleteResourceForm($name);
        if ($form->exists()) {
        ...
        $content = $this->render(
            ...
            $this->fileSystem->deleteResourceInputFilter($name)->getComponent(),
        );
    ...
    public function render(Component $form, Component $entity, Component $inputFilter): string
    {
        $class = (new ClassFile($form->getNamespace(), $form->getClassName()))
            ...
            ->useClass(Import::LAMINAS_FORM_ELEMENT_CHECKBOX)
            ...
            ->setComment(<<<COMM
/**
 * @phpstan-import-type Delete{$entity->getClassName()}DataType from {$inputFilter->getClassName()}
 * @extends AbstractForm<Delete{$entity->getClassName()}DataType>
 */
COMM);
