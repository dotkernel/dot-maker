...
class GetDeleteResourceHandler extends AbstractType implements FileInterface
{
    /**
    ...
    public function render(Component $handler, Component $entity, Component $form, Component $serviceInterface): string
    {
        ...
        \$this->{$form->toCamelCase()}->setAttribute(
            'action',
            \$this->router->generateUri('{$entity->toKebabCase()}::delete-{$entity->toKebabCase()}', ['id' => {$entity->getVariable()}->getId()->toString()])
        );
